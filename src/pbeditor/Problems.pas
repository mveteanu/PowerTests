...
type TProblem = class(TObject)
 public // private
    ...
    AnswerType  : Integer;  // de tip 0=TCheckBox ; 1=TEdit ; 2=TComboBox
...
begin
 ...
 else if AnswerType = 1 then for i:=1 to NrAnswers do UserAnswers[i]:=''
 else if AnswerType = 2 then for i:=1 to NrAnswers do UserAnswers[i]:='-1';
 ...
 if ts[i]<>'' then
  begin
    ...
    if ts2[0]='.random' then NrPbRandom:=strtoint(ts2[1])      // {**} Sa verific daca ts2[1] este numar intreg
        ...
        if ts2.count>1 then Path:=ExtractFilePath(paramstr(0))+ts2[1]
                       else begin getdir(0,ps);Path:=ps;end
    else if ts2[0]='.problem' then inc(NrProblems);
  ...
  begin
    ...
    if ts2[0]='.random' then NrPbRandom:=strtoint(ts2[1])      // {**} Sa verific daca ts2[1] este numar intreg
        ...
        if ts2.count>1 then begin
                              Path:=ExtractFilePath(paramstr(0))+ts2[1];
                              chdir(path);
