unit ordlg_GoodInstallForm;

{ This form is displayed in the dialog sequence that appears when the installation went
smoothly. }
...
uses
  ...
  Forms, Dialogs, StdCtrls, ExtCtrls, Buttons, cmp_TScalingForm, dlgcmp_TBaseDlg,
  cmp_TInstallContainer, cmp_TTranslateControls;

type
  TGoodInstallForm = class(TBaseInstallForm)
    Panel1: TPanel;
    Panel3: TPanel;
    Bevel1: TBevel;
    Panel2: TPanel;
...
var
  GoodInstallForm: TGoodInstallForm; { This value should never be used for anything because it will not point to the dialog }
...
constructor TGoodInstallForm.Create(AOwner: TComponent);
begin
  ...
  DM := LinkToManager(dmAfterGoodInstall);
  ...
  the properties below to control how you form is processed when run under different languages:
...
procedure TGoodInstallForm.FormCreate(Sender: TObject);
begin
...
procedure TGoodInstallForm.RefreshInformation;
begin
...
procedure TGoodInstallForm.BackBtnClick(Sender: TObject);
begin
  Previous; { Previous is a method of the TBaseInstallForm component }
end;

procedure TGoodInstallForm.ProceedBtnClick(Sender: TObject);
begin
  Next; { Next is a method of the TBaseInstallForm component }
end;

function TGoodInstallForm.Skip: Boolean;
begin
...
initialization
  ...
  GoodInstallFormBIFEntry := AddToBIFList(TGoodInstallForm,Nil,Nil);
